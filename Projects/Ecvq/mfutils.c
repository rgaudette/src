...
**
**  $Log: mfutils.c,v $
**  Revision 1.6  1996/03/20 02:59:13  root
**  *** empty log message ***
**
...
int getMatHeader(FILE *matFile, MatlabVar *Header) {
     ...
     **/
    nBytes = fread(&(Header->nRows), sizeof(int), 1, matFile);
    if(nBytes == 0) return(-1);

    nBytes = fread(&(Header->nCols), sizeof(int), 1, matFile);
    if(nBytes == 0) return(-1);

    nBytes = fread(&(Header->flgImag), sizeof(int), 1, matFile);
    if(nBytes == 0) return(-1);

    nBytes = fread(&(Header->lenName), sizeof(int), 1, matFile);
    if(nBytes == 0) return(-1);
     ...
     **/
    nBytes = fread(Header->Name, sizeof(char), Header->lenName, matFile);
    if(nBytes == 0) return(-1);
...
int putMatVariable(FILE *matFile, Matrix OutMatrix, char *Name) {
    ...
    int nBytes, idxRow, idxCol, lenName;
    ...
    nBytes = fwrite(&temp, sizeof(int), 1, matFile);
    if(nBytes < 1) return(0);

    nBytes = fwrite(&(OutMatrix.nRows), sizeof(int), 1, matFile);
    if(nBytes < 1) return(0);

    nBytes = fwrite(&(OutMatrix.nCols), sizeof(int), 1, matFile);
    if(nBytes < 1) return(0);
    ...
    nBytes = fwrite(&lenName, sizeof(int), 1, matFile);
    if(nBytes < 1) return(0);

    /**
     **  Write out the name of the variable
     **/
    nBytes = fwrite(Name, sizeof(char), lenName, matFile);
    if(nBytes < 1) return(0);
    ...
    for(idxCol = 0; idxCol < OutMatrix.nCols; idxCol++) {
        for(idxRow = 0; idxRow < OutMatrix.nRows; idxRow++) {
            ...
            nBytes = fwrite(&dtemp, sizeof(double), 1, matFile);
            if(nBytes < 1) return(0);
        }
    }
