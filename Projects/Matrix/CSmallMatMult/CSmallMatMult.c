...
int main(int argc, char *argv[]) {
    ...
    int m_min, m_max, m_step, iSize, iLoop;
    double trueClockRes, et, MFLOPS;
    clock_t start, finish, lstart, lfinish;
    FILE *resultsFile;
    ...
    //
    //  CSmallMatMult results.txt min_size max_size [m_step]
    //
    if(argc < 4){
        Usage();
        exit(-1);
    ...
    for(iSize=m_min; iSize<=m_max; iSize += m_step){
        ...
        //  Write results
        //
        et = (double)(finish - start -(lfinish - lstart)) / CLOCKS_PER_SEC;
        MFLOPS = NLOOPS * (matX.m * (matX.n + (matX.n - 1.0)) * matY.n) / 1.0E6;
...
void Usage(void){
    fprintf(stderr, "CSmallMatMult file m_min m_max [m_step [nLoops]]\n\n");
