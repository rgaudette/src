...
TEST_F(Test_SHT2D, set_shrinkage_weights)
{
  int n_levels = 5;
  int n_rows = 64;
  int n_cols = 128;
  StationaryHaarTransformer2D sht_2d(n_levels, n_rows, n_cols);
  float global_weight = 0.36F;
  float init[] = {1.0F, 1.0F, 1.0F, 1.0F, 1.0F};
  vector<float> subband_weight(init, init + 5);
  vector<float> diagonal_weight(init, init + 5);
  float pixel_gain = 0.36F;
  sht_2d.set_shrinkage_weights(1.5F, subband_weight, diagonal_weight, pixel_gain);
}
