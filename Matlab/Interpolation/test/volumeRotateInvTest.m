%volumeRotateInvTest   Test the volumeRotateInv function
function volumeRotateInvTest
...
rotVol = volumeRotateInv(vol, [45 0 0] * pi / 180, [], 'linear', 0);
figure(2)
colormap(cmap)
stackGallery(rotVol);


% Compare a forward rotation to an inverse rotation
...
% Inverse of the order above
rotVol = volumeRotateInv(vol, [90 -90 45] * pi / 180, [], 'linear', 0);
figure(4)
colormap(cmap)
stackGallery(rotVol);

% Perform a forward then an inverse of the negative to prove they are
% inverses
euler = rand(1, 3) * 2 * pi;
rotVol = volumeRotateInv(volumeRotate(vol, euler), -euler);
figure(5)
colormap(cmap)
stackGallery(rotVol);
