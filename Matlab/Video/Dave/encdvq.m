...
% The wavelet/VQ still image compression algorithm resides in
% the forward path of the encoder rather than JPEG.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
eval(['[video1_comp, comp] = vq_bit_alloc2(',video(1,:),',3,1.6);']);
...
%    eval(['[mot_vec_x,mot_vec_y] = overlap_motion(',video(i-1,:),',',video(i,:),');']);
    ...
    [residual_comp, comp] = vq_bit_alloc2(residual,6,.001);
    frame_comp(i)=comp;
