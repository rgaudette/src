...
% value = (xdim/block_size_xdim)^2*(bits/motion vector)/(8 bits/byte)
% See overlap.m function for computing this
mot_vec_bytes = 96;
...
eval(['[video1_comp, comp] = vq_bit_alloc2(',video(1,:),',3,2.0);']);
...
%    eval(['[mot_vec_x,mot_vec_y] = non_overlap(',video(i-1,:),',',video(i,:),');']);
    ...
    [residual_comp, comp] = bi_tcq_bit_alloc2(residual,6,.001);
    frame_comp(i)=comp;
...
save bi_tcq_over_8 compression frame_comp psnr_vec movie_handle; 