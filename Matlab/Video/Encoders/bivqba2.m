%BIVQBA2        Bi-Orthogonal VQ using the "second" bit-allocation strategy.
%
%   [rec_img, comp, Indices, nIndices, idxCB] = 
%       bi_vq_bit_alloc2(image, N, b,flgTime)
%
...
%
%   b           The average bit rate used to code the image.
%
...
dim1 = size(image);
LL = img_dec1(1:dim1(1)/2,1:dim1(2)/2);
dim = size(LL);
...
for i=1:N  
    ...
    r = b * ones(1,4) + 0.5 / log10(2) * log10(var ./ geom_mean);
...
for k = 1:(length(bit_assign)-3)
    ...
    %%
    idxRate = find((bit_assign(k) >= interval(1:length(interval-1))) & ...
                   (bit_assign(k) <  interval(2:length(interval))) );
    if idxRate == [],
    ...
    if ((bit_assign(k)>0) & (bit_assign(k)<8)) 
        %%
        %%   Normalize the coefficients, computing the mean and standard deviation
        %%
        ...
        idxCW = fsvq(reshape(subim_vect, vect_dim, nVects), trVQ);
...
if flgTime > 0,
    et = etime(clock, AlgStart); disp(['  BIVQBA2 total Elapsed time : ' num2str(et)]);
end
