%BIECVQB1       Bi-Orthogonal ECVQ using the "first" bit-allocation strategy.
%
%   [rec_img, comp, Indices, nIndices, idxCB] = biecvqb1(image, N, b,flgTime)
%
...
%
%       BIECVQB1 computes an N level bi-orthogonal wavelet decomposition of
...
%%
%%  Decompose the image into subbands measureing the variance and computing
%%  the bit rate for each subband
%%
for i=1:N,
    if flgTime > 1,
...
%%
[cdbk_rate cdbks Lambda] = ecvqggcb;
...
for k = 1:(length(bit_assign)-3)
    ...
    if ((bit_assign(k)>0) & (bit_assign(k)<8)) 
        ...
        %%   - The codebooks stored in Dave's files are transposed
        %%
...
%%
b = TotalBits / (szImage(1) * szImage(2));
comp = 8 / b;
