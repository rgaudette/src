...
pmi_scat = sphcntr200(:,11) .* exp(j*sphcntr200(:,12)*pi/180);
...
legend('My spherical code', 'Born', 'PMI')
...
title('Scattered Field Phase Comparison')
legend('My spherical code', 'Born', 'PMI')

figure(2)
clf
subplot(2,1,1)
plot(x, abs(phi_scat) ./ abs(born_scat));
xlabel('X position (cm)')
ylabel('Amplitude')
title('Spherical: Born Amplitude Ratio')

subplot(2,1,2)
plot(x, (angle(phi_scat) - angle(born_scat)) * 180 ./ pi);
xlabel('X position (cm)')
ylabel('Phase (degrees)')
...
clf
subplot(2,1,1)
plot(x, abs(phi_scat) ./ abs(pmi_scat));
xlabel('X position (cm)')
ylabel('Amplitude')
title('Spherical : PMI Amplitude Ratio')

subplot(2,1,2)
plot(x, (angle(phi_scat) - angle(pmi_scat)) * 180 ./ pi);
xlabel('X position (cm)')
ylabel('Phase (degrees)')
