...
%
%   See Also
%      BestBasis, PlotBasisTree- sum(sum(p .* log(eps+p)));
%    
    ...
    D  = L-1;
    %
    if strcmp(lower(Entropy), 'hamid')
        for d=0:D,
            for b=0:(2^d-1),
              CosPacket = (pkt(packet(d,b,n),d+1) ./ss);
              Seq = par(packet(d,b,n));
              tree(node(d,b)) = hamidc(CosPacket, Seq);
            end
        end
    elseif strcmp(Entropy,'Entropy'),
        for d=0:D,
            for b=0:(2^d-1),
