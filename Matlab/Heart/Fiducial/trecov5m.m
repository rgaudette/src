%TRECOV5M       Estimate the recovery time of a number of ECG leads.
%
%   [idxRecov Peaks Regions Decomp] = trecov5m(Leads, nDecomp, nWindow,
%                                              ScaleStop)
%
...
%
%
%        TRECOV5M uses Mallat's wavelet deomposition algorithm and works up
...
%   thresholded by a scaled by estimate of the the noise std. TRECOV5M modifies
%   TRECOV4M by using peaks instead of max to find the max in the search region 
%   at each scale that does not include the end points.
%
...
for iLead = 1:nLeads,
    ...
    vThresh = 3  * std(Leads(iLead, nSamples-20:nSamples));
...
%        nFilt = 4 * 2^(iScale - 1) + 2^iScale + 2;
        ...
        %%
        %%  Find the local maximum within the region, label this as the current
        %%  estimate.
        %%
        [PeakVals idxPeaks] = peaksrch(Region);
        if isempty(PeakVals)
            [Val idxLocalMax] = max(Region);
        else
            [Val idxBigPeak] = max(PeakVals);
            idxLocalMax = idxPeaks(idxBigPeak);
        end
