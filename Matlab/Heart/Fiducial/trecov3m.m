%TRECOV3M       Estimate the recovery time of a number of ECG leads.
%
%   [idxRecov Peaks Regions Decomp] = ...
%   trecov3m(Leads, nDecomp, nWindow, ScaleStop)
%
...
%               should represent a different lead, each column a different
%               time instant.
%
...
%
%
%        TRECOV3M uses Mallat's wavelet deomposition algorithm and works up
%   from coarser resoltions to finer to close-in on the recovery time 
%   (max dv/dt) near the peak of the T-wave.  This routine adds a median
...
for iLead = 1:nLeads,
    ...
    %%
    %%  Limit search to last local minimumn, this should correspond to the
    %%  falling edge of the T-wave
    %%
    ...
    for iScale = nDecomp-1:-1:ScaleStop
        ...
        %%
        %%  Compute the support of the decomposition filter for the current
        %%  scale
        %%
