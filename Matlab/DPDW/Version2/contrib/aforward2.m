...
r_r_dot_r_s=(r_s.x-r_obj.x)*(r_obj.x-r_r.x)+(r_s.y-r_obj.y)*(r_obj.y-r_r.y)+(r_s.z-r_obj.z)*(r_obj.z-r_r.z);
mag_r_r_and_r_s=sqrt((r_s.x-r_obj.x)^2+(r_s.y-r_obj.y)^2+(r_s.z-r_obj.z)^2)*sqrt((r_obj.x-r_r.x)^2+(r_obj.y-r_r.y)^2+(r_obj.z-r_r.z)^2);
...
for c=0:l
   ...
   phi_scatt=phi_scatt+A*sp_besselh(c,k_out*r)*mod_Y(c,theta);
end;
...
coef1=-i*v*(S_AC*k_out/D_out)*sp_besselh(l,k_out*r_s.z)*mod_Y(0,pi);
