%WSIRT_PERF
%
%  [WTrueErr, WResid, TrueErr, Resid] = wsirt_perf(A, x, nIter, vSNR)
%
...
function [WTrueErr, WResid, TrueErr, Resid, TeClean, ResidClean] = wsirt_perf(A, x, nIter, vSNR)
...
%%
xhat_clean = sirt(A, b_true, xo, nIter);
...
for iSNR = 1:nSNR
    ...
    A_w = diag(w) * A;
    b_noise_w = w .* b_noise;
    [xhat_w] = sirt(A_w, b_noise_w, xo, nIter);
