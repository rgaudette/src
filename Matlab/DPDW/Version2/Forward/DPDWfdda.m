...
%
%   [A Phi_Inc] = dpdwfdda(CompVol, mu_sp, mu_a, nu, f, pSrc, pDet)
%
...
function [A, Phi_Inc] = dpdwfdda(CompVol, mu_sp, mu_a, nu, f, pSrc, pDet)

%%
%%  Compute the PDE parameters from the media parameters
%%
D = nu / (3 * (mu_sp + mu_a));
k = sqrt(-nu / D * mu_a  + j * 2*pi*f / D);

fprintf('D = %e\n', D);
fprintf('Re{k} = %f cm^-1\n', real(k));
fprintf('Im{k} = %f cm^-1\n', imag(k));

[A Phi_Inc] = hlm3ptborn1nb(CompVol, k, pSrc, pDet);
