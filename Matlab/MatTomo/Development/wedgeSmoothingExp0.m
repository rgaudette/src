...
tiltsInterp = [-70:0.25:70];
zEnd = zeros(length(idxY), 1);
radInterp = interpn(idxY', [min(tiltsInterp) tilts max(tiltsInterp)], ...
                    [zEnd rad zEnd], ...
                    idxY', tiltsInterp, 'linear');
...
figure(8)
set(8, 'position', get(2, 'position'));
imagesc(tiltsInterp, idxY, radInterp);
colormap(gray(256))
