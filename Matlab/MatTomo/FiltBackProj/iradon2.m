...
%   $Revision: 1.1 $  $Date: 2004/01/04 21:30:32 $
...
% Frequency domain filter the projections
p = freqFilter(p, H, len);

% backprojection
img = backProject(p, len, theta, interp, N);

%%
%%  Ramp filter the projections in the frequency domain 
%%
function p = freqFilter(p, H, len)
...
%%
%% Backproject the pojections into the space domain
%%
function img = backProject(p, len, theta, interp, N)
...
elseif strcmp(interp, 'linear')
  ...
  for i=1:length(theta)  
      ...
      weight = (t-a);
      idx = a + ctrIdx;
      img = img + weight .* proj(idx+1) + (1 - weight) .* proj(idx);
  end
  profile off
