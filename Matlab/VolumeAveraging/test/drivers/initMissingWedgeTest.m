...
% Mask the volume to simulate the missing fourier domain wedge
VOL = fftshift(fftn(volume));
tiltRange = [-60 60];
mask = wedgeMask(tiltRange, size(VOL));
VOL = VOL .* mask;
volume = real(ifftn(ifftshift(VOL)));
