...
nParticles = 10;
szFullRef = [32 32 32];
bufferDist = szFullRef(1) - length/2 + 2 * 6;
...
[volume, reference, modParticle, motiveList] = ...
  randomTestVolume(szVol, nParticles, length, width, bufferDist, initOrient);
...
save(mrcVolume, 'rTransformTest.mrc');
save(MRCImage(fullRef), 'rTransformTest_InitRef.mrc');
write(modParticle, 'rTransformTrue.mod');
tom_emwrite('rTransformTest_MOTL_1.em', tom_emheader(motiveList));