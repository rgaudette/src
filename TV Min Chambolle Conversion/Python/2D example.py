""" A simple 2D example of Chambolle based TV minimization using the scikit.image.restoration code and a simpler
...
def main() -> None:
    ...
    nprand.seed(0)
    simple_rand_im = nprand.rand(4, 5)
    denoised = denoise_tv_chambolle(simple_rand_im, weight = 0.1, n_iter_max = 3, eps = 0.0)
    denoised_2D = _denoise_tv_chambolle_2D(simple_rand_im, weight = 0.1, n_iter_max = 3)

    try:
        nptest.assert_array_equal(denoised, denoised_2D)
...
def _denoise_tv_chambolle_nd(im, weight=0.1, eps=2.e-4, n_iter_max=200) -> np.ndarray:
    """Perform total-variation denoising on n-dimensional images.
...
def _denoise_tv_chambolle_2D(im: np.ndarray, weight: float=0.1, n_iter_max: int=200)  -> np.ndarray:
    """Perform total-variation denoising on 2-dimensional images. This is a modified version specifically for 2D images
    that uses simple, explicit indexing.
    ...
    ndim = 2
    p = np.zeros((2, ) + im.shape, dtype=im.dtype)
    g = np.zeros_like(p)
    d = np.zeros_like(im)
    i = 0
    while i < n_iter_max:
