...
function osqp_lasso(
    ...
    P = vcat(spzeros(n, 2 * n + m), hcat(Omn, Im, Omn), spzeros(n, 2 * n + m))
    if verbose && n + m < n_print_limit
        @printf("P:%s\n", P)
    end

    q = vcat(zeros(n + m), γ * ones(n))

    A_model = vcat(hcat(A, -Im, Omn), hcat(In, Onm, -In), hcat(In, Onm, In))
    if verbose && n + m < n_print_limit
        @printf("A_model:%s\n", A_model)
    end

    l = vcat(b, -Inf * ones(n), zeros(n))
    u = vcat(b, zeros(n), Inf * ones(n))
...
b = A * x_true
γ = 1.0
x_est, results = osqp_lasso(A, b, γ; x_true = x_true, verbose = true)
