...
**
**  Usage:
**    lbg training.mat variable codebook.mat
**
**  Description:
...
**    * Changed comand line structure, the program now takes the code vector size, the
**    maximum number of code bits and the number of training vectors to use.
**
...
int main(int argc, char *argv[]) {
    ...
    /**
     **  Check for sufficient number of command line args.
     **/
    if(argc < 8) {
        fprintf(stderr, "lbg: Not enough command line arguments\n\n");
        Usage();
        exit(-1);
    }

    /**
     **  Copy command line arguments into necessary variables
     **/
    strcpy(DataFile, argv[1]);
    strcpy(DataVar, argv[2]);
    strcpy(CodeFileBase, argv[3]);
    strcpy(RateDistFile, argv[4]);
    
    nElem = atoi(argv[5]);
    nCodeBitsMax = atoi(argv[6]);
    nTrainingVecs = atoi(argv[7]);
